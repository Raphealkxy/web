...
{   if(!document.getElementById("placeholder")) return false;
	...
	var picture=document.getElementById("placeholder");
	picture.setAttribute("src",source);
	if(document.getElementById("description"))
	{
	var text=wps.getAttribute("title");
	var text1=document.getElementById("description");
...
function preparePlaceholder()
{
	...
	if(!document.getElementById) return false;
	 ...
	 description.appendChild(desctext);//插入本属性；
	 var gallery=document.getElementById("imagegallery");
...
function displayAbbreviations()
{
	...
	var defs = new Array();//关联数组；
	for(var i=0;i<Abbrs.length;i++)
	  {
		  var current_abbrs=Abbrs[i];
		  if(current_abbrs.childNodes.length<1) continue;//判断子节点的个数；文本节点也是属性节点；
		  var definition =current_abbrs.getAttribute("title");
		  var key=current_abbrs.lastChild.nodeValue;
		  defs[key]= definition;
	 }
...
function achieveFocus()
{
	if(!document.getElementById||!document.getElementsByClassName||!document.getElementsByTagName) return false;
